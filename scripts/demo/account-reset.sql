...
-- Additional required psql variables: expected_database, apply_demo_reset=YES.
-- The account mode check below is mandatory even with an operator acknowledgement.
CREATE TEMP TABLE demo_reset_scope ON COMMIT DROP AS
SELECT :'expected_database'::text expected_database, :'apply_demo_reset'::text acknowledgement;
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM demo_reset_scope WHERE expected_database=current_database()
        AND acknowledgement='YES' AND length(expected_database)>0) THEN
    RAISE EXCEPTION 'Explicit DEMO reset database/acknowledgement mismatch';
  END IF;
END $$;
